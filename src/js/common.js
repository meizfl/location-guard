	...
	if(!Browser.capabilities.needsPAManualHide()) {
		...
		browser.tabs.onUpdated.addListener(function(tabId, info) {
			...
			if(info.status == 'loading') {
				// MV3: pageAction removed; nothing to hide
...
Browser.rpc._listener = function(message, sender, replyHandler) {
	...
	if(res instanceof Promise) {
		// MV3: return the Promise directly so Chrome can resolve it.
		// Also call replyHandler for MV2-style callers using callback sendMessage.
		res.then(replyHandler);
		return res;  // returning a Promise keeps the channel open in MV3
	} else {
		replyHandler(res);
		return false;
	}
...
Browser.rpc.call = async function(tabId, name, args) {
	...
	if(tabId) {
		// content script target: use callback form, suppress lastError
		return new Promise(resolve => {
			browser.tabs.sendMessage(tabId, message, function(response) {
				void browser.runtime.lastError; // suppress "no receiver" error
				resolve(response);
			});
		});
	} else {
		// background/service-worker target: use Promise form (MV3)
		try {
			return await browser.runtime.sendMessage(message);
		} catch(e) {
			// service worker may not be listening yet; swallow the error
			return undefined;
		}
	}
}
...
Browser.gui._refreshPageAction = function(tabId, info) {
	// MV3: pageAction is removed; fall back to action API
	return Browser.gui._refreshBrowserAction(tabId, info);
}
...
Browser.gui._refreshBrowserAction = function(tabId, info) {
	return new Promise(resolve => {
		...
		}).then(resolve);		// MV3: setIcon returns a Promise
	});
}
...
Browser.capabilities.permanentIcon = function() {
	...
	// MV3 always uses action (always permanent icon)
	return true;
}
...
Browser.log = function() {
	...
	//
	// getBackgroundPage() removed in MV3 (service workers)
}
...
if(typeof(browser) === 'undefined')
	(typeof self !== 'undefined' ? self : window).browser = chrome;
